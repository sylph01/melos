...
vectors = JSON.load_file('test_vectors/passive-client-random.json')
...
vectors.each_with_index do |vec, vec_index|
  ...
  vec['epochs'].each do |epoch_info|
    puts "Epoch #{group_context.epoch}: "
    ...
    if commit_msg.public_message
      ...
      # puts to_hex Melos::Struct::RatchetTree.root_tree_hash(suite, ratchet_tree)
